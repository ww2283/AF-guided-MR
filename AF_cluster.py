...
def run_colabfold(input_dir, output_dir, reference_pdb, num_recycle=5, num_models=1):
    ...
    for a3m_file in a3m_files:
        ...
        if result.returncode != 0:
            ...
            process_predicted_structure(pdb_file, ref_obj, af_cluster_selectives_dir)
...
if __name__ == "__main__":
    ...
    inference_dir = os.makedirs("predictions", exist_ok=True)
    # save clustered sequences in separate FASTA files
    write_clusters(cluster_labels, IDs, seqs, args.output_dir)

    saving_dir = os.getcwd()

    # Run ColabFold on the clustered MSA files
    run_colabfold(args.output_dir, saving_dir , args.reference_pdb)
